...
#if FABLE_COMPILER
    internal new : filename: string * errors: FSharpErrorInfo[] * scopeOptX: TypeCheckInfo option * dependencyFiles: string list * builderX: IncrementalBuilder option * reactorOpsX:IReactorOperations -> FSharpCheckFileResults
#endif
...
#if FABLE_COMPILER
    internal new : projectFileName:string * keepAssemblyContents: bool * errors: FSharpErrorInfo[] * details:(TcGlobals*TcImports*Tast.CcuThunk*Tast.ModuleOrNamespaceType*TcSymbolUses list*TypeChecker.TopAttribs option*CompileOps.IRawFSharpAssemblyData option * ILAssemblyRef * AccessibilityLogic.AccessorDomain * Tast.TypedImplFile list option * string list) option * reactorOps: IReactorOperations -> FSharpCheckProjectResults
#endif
...
module internal Parser =
    ...
    val internal ParseOneFile : ctok:CompilationThreadToken * source:string * matchBracesOnly:bool * reportErrors:bool * mainInputFileName:string * projectSourceFiles:string list * tcConfig:TcConfig -> FSharpErrorInfo [] * (range * range) [] * ParsedInput option * bool
...
type internal FSharpChecker =
    ...
    static member Create : ?projectCacheSize: int * ?keepAssemblyContents: bool * ?keepAllBackgroundResolutions: bool  * ?legacyReferenceResolver: ReferenceResolver.Resolver -> FSharpChecker
