        ...
        let writeMetadata (dllInfo: ImportedBinary) =
            ...
            try
                let args: ILBinaryWriter.options = {
                    ...
                    signer = None
                    dumpDebugInfo = false }
                ILBinaryWriter.WriteILBinary (outfile, args, ilModule)
            with Failure msg ->
                printfn "Export error: %s" msg

        let! dllinfos, ccuinfos = frameworkTcImports.RegisterAndImportReferencedAssemblies (ctok, tcResolutions.GetAssemblyResolutions())
        dllinfos |> List.iter writeMetadata
        let! dllinfos, ccuinfos = frameworkTcImports.RegisterAndImportReferencedAssemblies (ctok, tcAltResolutions.GetAssemblyResolutions())
        dllinfos |> List.iter writeMetadata
#endif
