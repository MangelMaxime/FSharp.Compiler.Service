...
type InteractiveChecker internal (tcConfig, tcGlobals, tcImports, tcState, ctok, reactorOps) =
    ...
    member x.ParseAndCheckScript (mainInputFileName, source) =
        ...
        | tcErrors, Parser.TypeCheckAborted.No scope, tcImplFiles ->
            ...
            let typeCheckResults = FSharpCheckFileResults (mainInputFileName, errors, Some scope, dependencyFiles, None, reactorOps)   
            let projectResults = FSharpCheckProjectResults (mainInputFileName, true, errors, Some(tcGlobals, tcImports, scope.ThisCcu, scope.CcuSig, [scope.ScopeSymbolUses], None, None, mkSimpleAssRef "stdin", tcState.TcEnvFromImpls.AccessRights, Some tcImplFiles, dependencyFiles), reactorOps)
